...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        observeState()
        Log.app.info("Dictator ready — hold \(Settings.shared.pushToTalkKey.displayName) to dictate")
    }

    /// `dictator://clear` and `dictator://show`, used by the legacy HTML dashboard and
    /// as a scriptable way to raise the window.
    func application(_ application: NSApplication, open urls: [URL]) {
        for url in urls where url.scheme == "dictator" {
            switch url.host {
            case "clear":
                RunLog.clear()
