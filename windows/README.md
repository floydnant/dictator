...
The Windows port of Dictator — push-to-talk dictation, on-device.
...
├─ global.json                    SDK pin
├─ src/
│  ├─ Dictator.Dictionary/          corrections + biasing          net10.0
│  ├─ Dictator.Abstractions/        the four platform interfaces   net10.0
│  ├─ Dictator.Core/                engine, segmenter, storage     net10.0
│  ├─ Dictator.Speech/              Parakeet via sherpa-onnx       net10.0
│  ├─ Dictator.Testing/             fakes for the interfaces       net10.0
│  ├─ Dictator.App/                 Avalonia UI                    net10.0
│  └─ Dictator.Platform.Windows/    the ONLY Win32 code            net10.0-windows
└─ tests/
   ├─ Dictator.Dictionary.Tests/    the shared vectors             24 tests
   ├─ Dictator.Core.Tests/          engine, chunking, storage      26 tests
   └─ Dictator.App.Tests/           headless Avalonia UI           13 tests
```
...
`Dictator.App` loads the platform layer **by reflection** rather than referencing it. A direct
...
```bash
cd windows
dotnet build Dictator.sln --no-incremental -warnaserror
dotnet test  Dictator.sln
```

**On macOS or Linux** — use the solution filter. `Dictator.Platform.Windows` targets
...
```bash
cd windows
dotnet test Dictator.CrossPlatform.slnf -c Release      # ~0.5s, 63 tests
```
