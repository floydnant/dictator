using Dictator.Core;
using Dictator.Dictionary;
using Shouldly;
using Xunit;

namespace Dictator.CoreTests;
...
{
    private readonly string _path = Path.Combine(Path.GetTempPath(), $"dictator-dict-{Guid.NewGuid():N}.txt");
...
{
    private readonly string _path = Path.Combine(Path.GetTempPath(), $"dictator-hist-{Guid.NewGuid():N}.jsonl");
...
{
    private readonly string _path = Path.Combine(Path.GetTempPath(), $"dictator-set-{Guid.NewGuid():N}.json");
